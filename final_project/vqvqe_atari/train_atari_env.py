...
LogTupStruct = namedtuple('Log', field_names=['episode_idx',
                                              ...
                                              'vqvae_buffer_size',
                                              'vqvae_recon_loss',
                                              'raw_buffer_store_count',
                                              'vqvae_buffer_store_count'
                                              ])
...
def write_image(agent_obj, out_path, num_img=8):
    ...
    # Ensure we can sampling
    if len(agent_obj.memory) < num_img or len(agent_obj.vqvae) < num_img:
        print('Not enough in storage to sample.')
        return None
    ...
    for episode_idx in range(args.num_episode):
        ...
        while True:
            ...
            if done:
                ...
                if args.write_img_period > 0 and (episode_idx+1) % args.write_img_period == 0:
                ...
                logtuple = LogTupStruct(episode_idx=episode_idx, steps=steps,
                                        ...
                                        vqvae_buffer_size=len(agent.vqvae), #[VQVAE] below all VAVAE logs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--vqvae-recon-threshold', type=float, default=15.0, metavar='S',
    ...
    parser.add_argument('--vqvae-latent-loss-weight', type=float, default=0.25, metavar='S',
                        help='latent loss weight of VQ-VAE (default: 0.25)')
    parser.add_argument('--vqvae-freeze-point', type=int, default=500000, metavar='S',
                        help='number of transitions to observe before freezing the VAE encoding (default: 500,000)')
    parser.add_argument('--write-img-period', type=int, default=-1, metavar='S',
                        help='number of episode to write out a batch of images (default: -1, no write)')
    parser.add_argument('--img-out-path', type=str, default='./out_imgs', metavar='S',
                        help='output directory to write the image to (default: ./out_imgs)')
